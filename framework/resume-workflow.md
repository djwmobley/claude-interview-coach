...
- **Project headers must follow one pattern throughout.** Use the `Title` line followed by a `Company | City, ST | 2019 – 2021` pipe line (the md_to_docx.py format) for every project entry. Never mix in a different header shape. No prefixes like "Flagship:".
...
7. **Header consistency:** Do all project headers follow the same `Title` / `Company | City, ST | 2019 – 2021` pattern?
