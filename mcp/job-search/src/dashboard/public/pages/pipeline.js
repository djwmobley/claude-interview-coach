...
export async function render(container, params, app) {
  ...
  async function load() {
    ...
    const activeSections = ACTIVE_GROUPS.map((g) => h('section', { className: 'pipeline-group' }, [
      ...
      byGroup[g].length === 0 ? emptyState({ message: `Nothing in ${groupLabel(g)} right now.` }) : h('div', { className: 'pipeline-group__rows' }, byGroup[g].map(renderRow)),
    ]));
    ...
    const outcomeSections = h('details', { className: 'pipeline-outcomes' }, [
      ...
      ...OUTCOME_GROUPS.map((g) => h('section', { className: 'pipeline-group' }, [
        ...
        byGroup[g].length === 0 ? emptyState({ message: `No ${groupLabel(g).toLowerCase()} listings.` }) : h('div', { className: 'pipeline-group__rows' }, byGroup[g].map(renderRow)),
      ])),
    ]);
  ...
  return { name: 'pipeline', refresh: load, teardown: () => off('dashboard:changed', onChanged) };
}
