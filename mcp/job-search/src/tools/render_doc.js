...
import { onDocumentLinked } from '../core/applications.js';
 ...
 * @param {{ listingId: number, kind: string, outputPath: string, root?: string }} input outputPath is
 *   renderDoc()'s own `output_path` (repo-relative, OS path separators, e.g. "output\\resumes\\X.docx"
 *   on Windows) or an already-absolute path.
...
export async function linkRenderedDocument(client, input) {
  ...
  const linkResult = await onDocumentLinked(client, input.listingId, docKind, linked.id, { actor: 'mcp' });
...
export const tool = {
  name: 'render_doc',
  description: 'Preflight a resume, cover letter, or cheat sheet markdown (em-dash, en-dash, scare quotes, buzzwords, problem-comparison reframe, resume structure, role inclusion, PMP wording, Jenkon title, output naming) and render the DOCX with the repo converters. Fails closed on any check; returns LOCKED when the DOCX is open in Word. Never opens the file. An optional listingId links the render to that listing\'s application and, for a resume, moves it from drafting to docs_ready.',
  schema,
  async handler(a, deps) {
    ...
    const linkOutcome = await deps.withClient((c) => linkRenderedDocument(c, { listingId: a.listingId, kind: a.kind, outputPath: /** @type {string} */ (result.output_path) }));
